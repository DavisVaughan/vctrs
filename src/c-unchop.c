...
r_obj* list_unchop(r_obj* xs,
  ...
  if (default_ != r_null && size == r_null) {
  ...
  if (size == r_null) {
    ...
    out_size = r_arg_as_ssize(size, "size");
  }
...
r_obj* ffi_list_unchop(r_obj* x,
  ...
  struct r_lazy default_arg_lazy = { .x = syms.default_arg, .env = frame };
...
r_obj* compute_default_index(
  ...
  bool* v_unused = r_vector_bool_begin(p_unused);

  // Mark `unused` locations in the boolean vector
  for (r_ssize i = 0; i < indices_size; ++i) {
    ...
    for (r_ssize j = 0; j < index_size; ++j) {
      ...
      v_unused[loc - 1] = false;
    }
  }

  // Convert boolean vector to integer locations, which is where `default` will go
  r_obj* out = r_vector_bool_which(p_unused);
