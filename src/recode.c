...
r_obj* vec_recode_values(
  ...
  if (multiple_from) {
    ...
    from = KEEP_N(vec_cast(from, x, p_from_arg, p_to_arg, call), &n_prot);
  }
  ...
  if (multiple_to) {
    out = vec_recode_values_with_multiple_to(
      x,
      from,
      to,
      default_,
    ...
    out = vec_recode_values_with_single_to(
      x,
      from,
      to,
      default_,
...
// we don't expect any errors from here on out
static
r_obj* vec_recode_values_with_multiple_to(
  ...
  r_obj* indices = KEEP(build_indices(x, from, x_size, from_size, multiple_from));

  // This might still be the user's `to` list
  to = KEEP(r_clone_referenced(to));
  ...
  r_obj* size = KEEP(r_int(r_ssize_as_integer(x_size)));
...
r_obj* vec_recode_values_with_single_to(
  ...
  r_obj* xs = KEEP_N(r_alloc_list(1), &n_prot);
  r_list_poke(xs, 0, to);

  r_obj* indices = KEEP_N(r_alloc_list(1), &n_prot);
  r_list_poke(indices, 0, loc_of_match);

  r_obj* size = KEEP_N(r_int(r_ssize_as_integer(x_size)), &n_prot);
...
r_obj* ptype_finalize(
  ...
  if (multiple_to) {
    ...
    int _;
    ptype = KEEP(vec_ptype2_params(ptype, default_, vec_args.empty, p_default_arg, call, &_));
