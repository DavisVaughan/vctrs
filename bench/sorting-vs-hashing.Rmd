...
However, `size = 10000` seems to already show `vec_unique_loc2()` being faster.
...
This benchmark shows how much better `vec_unique_loc2()` scales for large size and large number of groups. For doubles it is slower up front, but scales much better.
...
I am confident that the C level part of `vec_unique_loc2()` could gain a `sort_character = false` option that would do a much faster counting sort in order-of-first-appearance that utilizes the truelengths in a different way. It wouldn't sort strings at all, so should be very fast. This is what `cgroup()` does in `base::order()`, which is not currently implemented in `vec_unique_loc2()` because I didn't have a use for it until now. https://github.com/wch/r-source/blob/8d7ac4699fba640d030703fa010b66bf26054cbd/src/main/radixsort.c#L1051
...
- Extremely large difference, because `vec_unique_loc2()` is doing way too much work to actually sort the strings.
...
Same as before but with character columns. We do worse here because as mentioned before, we do too much work in `vec_unique_loc2()` right now with character vectors.
...
There is a point where we switch from counting sort to radix sort. For integers, it happens when we hit a _range_ of values exceeding 100,000. When there are many groups and we have a wide range, this transition isn't that impactful. But when we have a very small number of groups (here, 2), then that transition can be very big, and can make the dictionary approach of `vec_unique_loc()` faster.

I feel like this case is somewhat rare though.

```{r}
dict1 <- c(1L, INT_ORDER_COUNTING_RANGE_BOUNDARY - 1L)
dict2 <- c(1L, INT_ORDER_COUNTING_RANGE_BOUNDARY + 1L)
size <- 10 ^ (6:8)

df <- bench::press(
  size = size,
  {
    x <- sample(dict1, size, replace = TRUE)
    y <- sample(dict2, size, replace = TRUE)
    bench::mark(
      pre_boundary_order = vec_unique_loc2(x), 
      pre_boundary_dict = vec_unique_loc(x), 
      post_boundary_order = vec_unique_loc2(y), 
      post_boundary_dict = vec_unique_loc(y), 
      iterations = 10, 
      check = FALSE
    )
  }
