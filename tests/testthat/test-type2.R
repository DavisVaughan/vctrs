...
test_that("vec_typeof2() returns common type", {
  nms <- names(base_with_s3_empty_types)

  for (i in seq_along(base_with_s3_empty_types)) {
    this <- nms[[i]]

    for (j in seq_along(base_with_s3_empty_types)) {
      that <- nms[[j]]
      ...
      out <- vec_typeof2(base_with_s3_empty_types[[this]], base_with_s3_empty_types[[that]])
