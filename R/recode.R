...
#' @param x A vector.
#'
#' @param from Values present in `x` that are mapped to values in `to`.
#'
#'   Values present in `from` but not in `x` are ignored.
#'
#'   Values present in `x` but not in `from` are recoded to `default` when using
#'   `vec_recode_values()`, and will be left unchanged when using
#'   `vec_replace_values()`.
#'
...
#'   type of `to` and `default`, unless overriden by `ptype`.
#'
